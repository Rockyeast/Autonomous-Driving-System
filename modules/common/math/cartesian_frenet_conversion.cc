...
/**
 * 笛卡尔转frenet，和公式一样的
 * https://blog.csdn.net/qq_24561095/article/details/124791994?spm=1001.2014.3001.5502
*/
void CartesianFrenetConverter::cartesian_to_frenet(
  ...
  const double cross_rd_nd = cos_theta_r * dy - sin_theta_r * dx;  // 叉乘
  ptr_d_condition->at(0) =
      std::copysign(std::sqrt(dx * dx + dy * dy), cross_rd_nd);  // d的符号和叉乘符号一样，copysign函数后面的符号，前面是值
  ...
  ptr_d_condition->at(1) = one_minus_kappa_r_d * tan_delta_theta;  // d的一阶导数
  ...
  ptr_d_condition->at(2) =
      ...
      one_minus_kappa_r_d / cos_delta_theta / cos_delta_theta *
          (kappa * one_minus_kappa_r_d / cos_delta_theta - rkappa);  // d的2劫倒数

  ptr_s_condition->at(0) = rs;

  ptr_s_condition->at(1) = v * cos_delta_theta / one_minus_kappa_r_d;  //s一阶导
