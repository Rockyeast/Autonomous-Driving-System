...
std::unique_ptr<Stage> LaneFollowScenario::CreateStage(
  ...
  if (stage_config.stage_type() != ScenarioConfig::LANE_FOLLOW_DEFAULT_STAGE) {
    std::cout << "Follow lane does not support stage type: "
           << ScenarioConfig::StageType_Name(stage_config.stage_type()) << std::endl;
    return nullptr;
  }
  //LaneFollowStage的构造函数包含stage的构造函数，注册所包含的task
