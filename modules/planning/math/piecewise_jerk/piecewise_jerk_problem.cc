...
OSQPData* PiecewiseJerkProblem::FormulateProblem() {
  ...
  // 目标函数P矩阵
  // 这个目标函数矩阵的构造是有问题的，构造了一个下三角矩阵，但osqp0.4.1
  // 这个目标函数矩阵的构造是有问题的,是一个下三角矩阵，它并没有写成标准的对称阵的形式。
  // 标准对称阵形式，需要矩阵是对称的，即对称的元素相等，且均等于公式中对应系数的1/2。而apollo这种写法，只有下三角部分有值，且不需要取系数的1/2
  // 虽然数学上两种形式是等价的，但是在利用osqp求解的时候，低版本的osqp（apollo用的是低版本），会自动舍弃下三角部分，再将上三角部分复制到下三角中，强制形成一个对称阵。
  // 所以会引入误差。
  // 新版本osqp中，已经强制要求P矩阵为一个上三角矩阵

  // todo:把原系数改到上三角，并系数除于2
...
bool PiecewiseJerkProblem::Optimize(const int max_iter) {
  OSQPData* data = FormulateProblem();  // 构造二次规划三个矩阵和上下界
