...
void PiecewiseJerkSpeedProblem::CalculateKernel(std::vector<c_float>* P_data,
  ...
  const int kNumParam = 3 * n; // 第一个n内是s相关，第二个n是s‘相关，第三个n是s‘’
  ...
  // x(i)^2 * w_x_ref
  // s 偏离参考线惩罚项，(x - x_ref)^2 的展开项 x^2 部分，展开项的后面两个在g矩阵里
  for (int i = 0; i < n - 1; ++i) {
    columns[i].emplace_back(
        i, weight_x_ref_ / (scale_factor_[0] * scale_factor_[0])); // scale_factor_三个数都是1
    ++value_index;
  }
  // x(n-1)^2 * (w_x_ref + w_end_x) 
  // 展开项n-1的平方，第一第二个式子有同类项，合并就是(w_x_ref + w_end_x)
  ...
  // s''' 的另一部分
  for (int i = 0; i < n - 1; ++i) {
    columns[2 * n + i].emplace_back(2 * n + i + 1,  //columns是二维数组，所以插入的下标是一样的，在里边说明是哪一项就好
