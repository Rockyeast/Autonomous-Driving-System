...
class Obstacle {
 public:
  ...
  bool IsVirtual() const { return is_virtual_; } // 为了虚拟障碍物墙

  // 根据时间参数找障碍物轨迹上的点（不一定是已有的点，根据线性插值得到）
  common::TrajectoryPoint GetPointAtTime(const double time) const;

  // 根据传入的点坐标，获取根据障碍物长宽碰撞出的BoundingBox
