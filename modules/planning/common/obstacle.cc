...
Obstacle::Obstacle(const std::string& id,
  ...
  // (common::math::Polygon2d::ComputeConvexHull(polygon_points,
  //                                                   &perception_polygon_))
  ...
  for (int i = 1; i < trajectory_points.size(); ++i) {
    ...
    if (prev.relative_time() >= cur.relative_time()) {
      std::cout << "prediction time is not increasing."
             ...
             << "previous point: " << prev.ShortDebugString() << std::endl;
    }
    cumulative_s +=
...
common::TrajectoryPoint Obstacle::GetPointAtTime(
  ...
  const auto& points = trajectory_.trajectory_point();  //历史轨迹点集
  if (points.size() < 2) {  //轨迹点小于2，认为是静态障碍物
    ...
    auto it_lower =
        std::lower_bound(points.begin(), points.end(), relative_time, comp);  //std::lower_bound找到第一个大于等于relative_time的值，并返回下标
    ...
    return common::math::InterpolateUsingLinearApproximation(  //线性插值
...
bool Obstacle::IsValidPerceptionObstacle(const PerceptionObstacle& obstacle) {
  if (obstacle.length() <= 0.0) {
    std::cout << "invalid obstacle length:" << obstacle.length() << std::endl;
    return false;
  }
  if (obstacle.width() <= 0.0) {
    std::cout << "invalid obstacle width:" << obstacle.width() << std::endl;
    return false;
  }
  if (obstacle.height() <= 0.0) {
    std::cout << "invalid obstacle height:" << obstacle.height() << std::endl;
    return false;
  }
  if (obstacle.has_velocity()) {
    if (std::isnan(obstacle.velocity().x()) ||
      ...
      std::cout << "invalid obstacle velocity:"
             << obstacle.velocity().DebugString() << std::endl;
      return false;
    }
  }
  for (auto pt : obstacle.polygon_point()) {
    if (std::isnan(pt.x()) || std::isnan(pt.y())) {
      std::cout << "invalid obstacle polygon point:" << pt.DebugString() << std::endl;
      return false;
    }
  }
  return true;
  ...
  for (const auto& prediction_obstacle : predictions.prediction_obstacle()) {
    if (!IsValidPerceptionObstacle(prediction_obstacle.perception_obstacle())) {
      std::cout << "Invalid perception obstacle: "
             << prediction_obstacle.perception_obstacle().DebugString() << std::endl;
      continue;
    }
...
void Obstacle::AddLongitudinalDecision(const std::string& decider_tag,
  ...
  std::cout << decider_tag << " added obstacle " << Id()
         << " longitudinal decision: " << decision.ShortDebugString()
         << ". The merged decision is: "
         << longitudinal_decision_.ShortDebugString() << std::endl;
...
void Obstacle::AddLateralDecision(const std::string& decider_tag,
  ...
  std::cout << decider_tag << " added obstacle " << Id()
         ...
         << ". The merged decision is: "
         << lateral_decision_.ShortDebugString() << std::endl;
  decisions_.push_back(decision);
  decider_tags_.push_back(decider_tag);
}
